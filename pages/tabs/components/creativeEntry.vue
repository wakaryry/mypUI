<template>
	<view class="ce">
		...
		<view class="ce-grid">
			<myp-item v-for="(item,idx) in items" :key="idx" :icon="item.icon" :text="item.name" textType="text" iconType="warning" iconStyle="font-size: 60rpx;" space="20rpx" boxStyle="flex:1;height:180rpx;"></myp-item>
		</view>
	</view>
</template>
...
<script>
	export default {
		data() {
			return {
				items: [
					{icon: 'report', name: '创作首页'},{icon: 'edu', name: '创作学院'},{icon: 'subject', name: '创作日历'},{icon: 'task', name: '热门活动'},
				]
			}
		}
	}
