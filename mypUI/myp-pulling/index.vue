<template>
	<view class="myp-down-content" :style="mrStyle" @touchstart="downTouchstart" @touchmove="downTouchmove" @touchend="downTouchend">
		<view class="myp-loading-wrapper" :style="{height: downHeight + 'rpx'}">
			<slot name="down"></slot>
		</view>
		<slot></slot>
	</view>
...
<script>
	...
	export default {
		...
		computed: {
			...
			mrStyle() {
				let _style = "transform:translateY(" + this.downDeviation + 'px);'
				_style += 'transition-duration:' + this.downTransitionDuration + 'ms;'
				_style += 'height:' + this.height + 'rpx;'
				return _style
			}
		},
		methods: {
